...
const FileUpload = () => {
  ...
  return filename
    .replace(/\.png$/i, '')  // .png uzantısını kaldır
    .split(/[_\-\s]+/)       // Alt çizgi, tire ve boşluklara göre böl
    .filter(word => word.length > 2)  // Kısa kelimeleri filtrele
    .map(word => word.toLowerCase()); // Küçük harfe çevir
};
...
const readFileContent = async (file) => {
  try {
    ...
    if (extension === 'png') {
      ...
      return keywordsFromName.join(' ');  // Dosya adındaki kelimeleri boşlukla birleştirip metin olarak döndür
    }
  ...
  const analyzeFiles = async (files) => {
    try {
      // Her dosya için ayrı analiz yap
      ...
      // Hata durumunda her dosya için boş sonuç döndür
...
const handleUpload = async () => {
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      // Anahtar kelimeleri normalize et ve temizle
      const normalizedKeywords = analysis.keywords
        ...
        .filter(k => k && k.length > 0); // Boş ve null değerleri filtrele
    ...
    const dbOperations = uploadResults.map(async (result) => {
      ...
      // Anahtar kelimeleri kaydet (eğer varsa)
    ...
    // Tüm veritabanı işlemlerini bekleyerek tamamla
    await Promise.all(dbOperations);

    // 5. Sonuçları state'e kaydet (sadece ilk dosyanın sonuçlarını göster)
